...
def generate_cookie(user_id, oPHP = None):
    ...
    try:
        ...
        cursor.execute("select user_login, user_pass from %susers where ID = %s" % (WP_TABLE_PREFIX, '%s'), (user_id, ))
    ...
    try:
        if cookie:
            ...
            if int(expire) >= time.time():
                #Get the password slice
                username = username.replace("+", " ")
                cursor.execute("select ID, user_pass from %susers where user_login = %s" % (WP_TABLE_PREFIX, '%s'), (username, ))
